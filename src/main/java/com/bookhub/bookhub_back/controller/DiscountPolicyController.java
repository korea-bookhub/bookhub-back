...
public class DiscountPolicyController {
    ...
    //1) 할인 정책 생성
    @PostMapping
    public ResponseEntity<ResponseDto<DiscountPolicyDetailResponseDto>> createPolicy(
            @Valid @RequestBody DiscountPolicyCreateRequestDto dto){
        ResponseDto<DiscountPolicyDetailResponseDto> discountPolicy = discountPolicyService.createPolicy(dto);
        return ResponseEntity.status(HttpStatus.CREATED).body(discountPolicy);
    }

    //2) 할인 정책 수정
    @PutMapping("/{policyId}")
    public ResponseEntity<ResponseDto<DiscountPolicyDetailResponseDto>> updatePolicy(
            @PathVariable Long policyId,
            @Valid @RequestBody DiscountPolicyUpdateRequestDto dto){
        ResponseDto<DiscountPolicyDetailResponseDto> responseDto = discountPolicyService.updatePolicy(policyId, dto);
